...
public class AdminUserPutController extends BaseEntityPutController {
	...
	public Result put(@Valid @ModelAttribute("entity") CsmUser user, ServletRequest request) {
		final CsmUser oldUser = getOldEntity(request);
		adminUserService.putUser(user, oldUser);
		return HttpResults.success();
	}

	@ModelAttribute
	public <T, ID extends Serializable> void getEntity(String id, Model model, ServletRequest request) {
